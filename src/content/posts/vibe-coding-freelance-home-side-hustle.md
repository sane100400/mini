...
Cursor를 처음 쓸 때 효율이 가장 높았던 방식은 단계별로 요청하는 것이다. "랜딩페이지 전체 만들어줘"라고 한 번에 요청하면 결과가 두루뭉술하게 나온다. 반면 "제목 섹션부터 만들어줘, 제목은 AI 부업 30일 운영표로", "다음은 기능 설명 섹션, 카드 3개로 나눠줘", "마지막에 CTA 버튼 추가해줘"처럼 나누면 결과가 잡힌다.

수정도 마찬가지다. "전체적으로 더 예쁘게 해줘"보다 "h2 폰트 크기를 조금 키워줘", "CTA 버튼 배경색을 더 진하게 해줘", "모바일에서 이미지 아래로 텍스트 오도록 수정해줘"처럼 구체적으로 말해야 원하는 방향으로 간다.

## v0는 첫 화면 초안용으로 잘 맞았다
...
프롬프트는 짧게 쪼개는 게 요령이다. "멋진 랜딩페이지 만들어줘"라고 뭉뚱그려 던지면 결과도 뭉뚱그려져 돌아온다. "전자책 판매용 첫 화면을 만들고 제목은 AI 부업 30일 운영표로 해줘", "가격 섹션은 카드 2개로 나눠줘", "모바일에서 버튼이 아래로 줄바꿈되게 해줘"처럼 단계별로 요청해야 결과가 잡힌다.
...
## 의뢰인에게 넘기기 전 최종 확인 항목

배포 링크를 보내기 직전에 확인하는 체크리스트를 만들었다. 이게 없으면 작은 실수가 납품 후 수정 요청으로 돌아온다.

- 모바일 화면에서 레이아웃이 깨지지 않는가
- 모든 버튼이 실제 링크로 연결되어 있는가
- 이미지가 지나치게 무겁지 않은가 (로딩 속도)
- 폼이 실제로 전송되는가
- 연락처, 이메일 주소가 의뢰인 것으로 들어갔는가
- "예시 텍스트"나 "Lorem ipsum" 같은 임시 문구가 남아 있지 않은가
- GitHub 저장소와 README가 업데이트되어 있는가

이 체크리스트를 돌리면 실제로 납품 후 발생하는 수정 요청의 절반 이상이 줄어든다.
