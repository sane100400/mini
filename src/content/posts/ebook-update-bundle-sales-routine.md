...
이미 전자책을 팔고 있다면 새 상품을 계속 찍어내기보다 기존 상품을 살아 있게 만드는 편이 낫다. 업데이트 로그를 남기고, 보너스 파일을 더하고, 블로그 글과 연결하고, 번들 구성을 바꾸는 식이다. 구매자는 파일 개수보다 "이 사람이 계속 고치고 있구나"라는 신호를 더 오래 기억한다.
...
## 판매 플랫폼 화면에서 필요한 요소를 본다

전자책 글에는 판매 플랫폼 화면이 있어야 한다. 상단 대표 이미지는 Gumroad 공개 홈 화면을 직접 캡처했다. 디지털 상품을 검색하고 판매를 시작하는 버튼이 보인다. 이런 화면을 넣는 이유는 전자책을 판다는 분위기를 내기 위해서가 아니다. 실제로 어디에서 팔리고, 어떤 식으로 디지털 상품으로 보이는지 확인하기 위해서다.

Gumroad 같은 플랫폼을 보면 전자책을 파일 하나로만 생각하면 부족하다는 것을 알 수 있다. 판매 페이지, 소개 문구, 업데이트 안내, 파일 구성, 결제 후 안내가 같이 필요하다.

국내 판매를 생각하면 크몽도 같이 봐야 한다. 크몽 공개 메인 화면에는 전자책과 AI 서비스 카테고리가 함께 보인다. 전자책이 단독 상품으로만 팔리는 것이 아니라 컨설팅, 템플릿, 원고 대행, 상세페이지 문구 같은 서비스와 붙을 수 있다는 점이 보인다.
...
블로그에 먼저 운영 기록을 쓰고, 반응이 좋은 글을 전자책에 정리한다. 반대로 전자책에 들어간 체크리스트 일부를 블로그에서 소개한다. 무료 글과 유료 파일이 같은 말을 반복하지 않게 하려면 역할을 나눠야 한다.

## 판매 문구는 기능보다 문제에서 시작해야 한다
...
AI로 판매 페이지 초안을 만들기도 한다. 하지만 최종 문구는 사람이 줄여야 한다. "효율적으로 수익화를 시작할 수 있습니다" 같은 문장은 너무 넓다. "첫 30일 동안 어떤 글을 쓰고 어떤 파일을 업데이트할지 정합니다"처럼 구체적으로 바꿔야 한다.
